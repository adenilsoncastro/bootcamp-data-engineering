...
def data_ingestor_fixture():
    return DataIngestor(writer=DataWriter, coins=[
        'test', 'how'], default_start_date=datetime.date(2021, 7, 1))


# Um patch sobrescreve um método de uma determinada classe
@patch('mercado_bitcoin.ingestors.DataIngestor.__abstractmethods__', set())
class TestIngestors:
    ...
    def test_load_checkpoint_no_checkpoint(self, data_ingestor_fixture):
        ...
        expected = datetime.date(2021, 7, 1)
    ...
    def test_update_checkpoint_updated(self, mock, data_ingestor_fixture):
        data_ingestor_fixture._update_checkpoint(
